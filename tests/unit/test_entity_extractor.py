...
class TestLightweightExtraction:
    def test_tech_terms(self):
        entities = extract_entities_lightweight(
            "I use Python and PostgreSQL for this project"
        )
        ...
        assert all(
            e.entity_type == "tech"
            for e in entities
            if e.name in ("python", "postgresql")
        )
    ...
    def test_mentions(self):
        ...
        assert "bob" in names
        assert all(
            e.entity_type == "person" for e in entities if e.name in ("alice", "bob")
        )

    def test_repo_pattern(self):
        entities = extract_entities_lightweight(
            "Check matrixorigin/matrixone for details"
        )
        names = {e.name for e in entities}
    ...
    def test_camel_case(self):
        entities = extract_entities_lightweight(
            "The GraphBuilder handles node creation"
        )
        names = {e.name for e in entities}
